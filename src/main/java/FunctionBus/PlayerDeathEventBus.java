...
public class PlayerDeathEventBus {
    ...
    public static void onPlayerDeadByPlugin(PlayerDeathEvent event) {
        ...
        ScoreBoardBus.setPlayerScore(killer.getName(), "kill", ScoreBoardBus.getPlayerScore(killer.getName(), "kill") + 1);
        ScoreBoardBus.setPlayerScore(killer.getName(), "total_kill", ScoreBoardBus.getPlayerScore(killer.getName(), "total_kill") + 1);
        ScoreBoardBus.setPlayerScore(player.getName(), "total_dead", ScoreBoardBus.getPlayerScore(player.getName(), "total_dead") + 1);
